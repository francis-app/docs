...
Currently, it's technically possible to assign clients Editor rights while using the 50% discount code. This set-up is trust-based. We reserve the right to review access levels and adjust discounts if misaligned. For example, if Editor rights are granted while a 50% discount is applied.
...
1. Duplicate the model you want to promote. Promoting a model to a template moves it to a new location – make sure to use a copy, not one still in use by a client.
...
If you want to demo Francis to potential clients, we can create a free demo organization for you with sample data. To get started, just email [support@francis.app](mailto:support@francis.app) and request a demo set-up.
