...
Use statistical when headcount is stable and salary increases follow a predictable annual pattern. It doesn't account for new hires or departures.
...
<Tabs>
  <Tab title="Driver-based">
    **Set up the sheet structure**
    ...
    For the salary section, group by department. A common approach adds a group per employee or role with rows for base salary, pension, and other employer costs. The exact structure depends on what your business tracks — you might group by role rather than individual, or use a single aggregated row per department if you don't need that level of detail.

    <Tip>If you use dimensions, you can break down actuals by department or employee without splitting GL accounts into separate rows. This is useful if your chart of accounts doesn't map cleanly to your headcount structure.</Tip>
