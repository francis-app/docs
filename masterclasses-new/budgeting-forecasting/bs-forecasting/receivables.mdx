...
Driver-based is the right default when revenue is the primary driver. Express the closing balance as a function of revenue and payment days. When revenue assumptions change, the receivables balance updates automatically.
...
Hardcoded is often the most accurate in practice. Payment delays, disputes, and changes in payment terms can make driver-based models less reliable. It is also the right approach when modeling a deliberate reduction in trade receivables to hit a cash target.
...
<Tabs>
  <Tab title="Driver-based">
    ...
    **Cash flow**
    ...
    The change in trade receivables flows automatically to the working capital section of the cash flow statement. An increase in the balance is a cash outflow; a decrease is a cash inflow. Francis calculates this from the balance sheet movement each period on a 12-month rolling basis. See [cash flow fundamentals](/masterclasses-new/budgeting-forecasting/cf-forecasting/cf-fundamentals) for how working capital movements feed the cash flow statement.
  </Tab>
  ...
  <Tab title="Hardcoded">
    Enter the expected closing balance for each month based on management estimates. This is often the most accurate approach: driver-based models assume payment timing follows a fixed formula, but delays, disputes, and changes in payment terms can make the actual balance deviate significantly.

    Hardcoded is also the right approach when modeling a deliberate reduction in trade receivables, for example a target to reduce payment days or a one-off collection drive. Enter the target balance directly rather than trying to back-solve a driver.

    Document the basis in a formula note. Trade receivables change with revenue and customer behaviour, so hardcoded values require a manual review each planning cycle.
  </Tab>
</Tabs>
