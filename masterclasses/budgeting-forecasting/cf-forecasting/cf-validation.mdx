...
<Steps>
  <Step title="Check GL account mappings">
  ...
  <Step title="Check for manual overrides">
    Are there manual overrides in the P&L or balance sheet that cause values to diverge from your accounting system source of truth?
  </Step>
  <Step title="Check P&L subtotals">
    ...
    Usual suspects:
    - **A new group isn't in the subtotal.** A group was added to the P&L but the subtotal formulas above it were never extended to include it, so net profit is missing those rows.
  </Step>
  ...
  <Step title="Check for non-GL line items">
    ...
    Usual suspects:
    - **A broken helper sheet.** A helper sheet adjusts the ERP numbers before they land in the P&L or balance sheet, and its formulas don't reconcile back to the source.
  </Step>
</Steps>
...
<Steps>
  <Step title="Check for cash journals in non-cash lines">
    Have any cash journal entries been posted to non-cash line items, causing them to be excluded from the cash flow statement? Look for journal entry descriptions that suggest cash activity: *invoice allocation*, *expense*. In each case the bank leg moves cash, but the other leg lands on a line item the cash flow statement excludes, so the movement never shows up.

    Usual suspects:
    - **Cash expense booked to depreciation.** Debit a P&L depreciation line item, credit bank. Cash drops, the cash flow statement doesn't show the outflow.
    - **Dividend received from a subsidiary.** Debit bank, credit investment in subsidiaries. Cash rises, the cash flow statement doesn't show the inflow.
    - **Dividend paid out.** Debit retained earnings, credit bank. Cash drops, but retained earnings movement is excluded from the cash flow statement because it's assumed to be net profit.
  </Step>
  <Step title="Check for non-cash journals in cash lines">
    Have any non-cash journal entries been posted to cash line items, causing them to be incorrectly included? Look for descriptions that indicate non-cash activity: *depreciation*, *subsidiary*, *adjustment entry*. These are pure accounting adjustments, but one leg lands on a line item the cash flow statement includes, so it reports a movement that never touched the bank.

    Usual suspects:
    - **Write-off of fully depreciated assets.** Credit the CAPEX asset account, debit accumulated depreciation. No cash moves, but the CAPEX side is included, so the cash flow statement shows an inflow that didn't happen.
    - **Depreciation credited straight to CAPEX.** Debit the P&L depreciation expense, credit the CAPEX account instead of a separate accumulated depreciation account. CAPEX is included in the cash flow statement, so every month's depreciation reads as a fixed asset movement.
  </Step>
</Steps>
