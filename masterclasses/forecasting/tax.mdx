...
Now that you’ve separated timely tax expenses from after-the-fact adjustments, you can start forecasting tax expenses. We typically see three stages:
...
2. Forecasting taxes based on cash payments. Tax expenses are recorded when payments are made, for example, an _aconto_ payment in March, another in November, and an adjustment at year-end. This approach reflects cash movements but can distort reported profitability throughout the year.
3. Forecasting taxes across months. Tax payable accounts are used on the balance sheet to separate expense recognition from cash outflow, giving a clearer view of both profitability and cash flow.

If you have taxable income and expect to incur tax liabilities, we recommend forecasting taxes month-by-month. It provides a more accurate view of your financial performance and cash effects. If you prefer forecasting based on cash payments, you can still apply the formulas outlined below.
...
Other methods include using actuals from prior years if operations are stable, applying a forecasted effective tax rate based on historical data or external advisor input, or incorporating specific tax adjustments.
