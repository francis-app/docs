...
<video controls className="w-full aspect-video rounded-xl" src="/videos/features/charts/combo.mp4" />

A combo chart plots values over time using more than one representation, typically columns for one measure and a line for another. Use it when you want to compare measures of different scale or unit on a single chart, such as revenue as columns against gross margin percentage as a line.
