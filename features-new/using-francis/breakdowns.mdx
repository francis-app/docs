...
Structure flows top-down: sections, groups, rows, and calculations you define on the parent sheet sync automatically to all subsheets. Calculations run locally on each subsheet against that subsheet's data. You build the structure once and every subsheet inherits it.
...
On a multi-entity model, apply the entity breakdown before the dimension breakdown. Applying a dimension breakdown first only affects the first entity and won't give you unified dimensions across entities.

Each breakdown layer multiplies the total number of subsheets you maintain. Two entities broken down by three dimension values gives six subsheets. Break each of those down by three further values and you have eighteen. Plan your breakdown structure before you start. Adding layers later means more subsheets to update, and removing a layer drops all forecasts entered on those subsheets.
...
<Note>
  When a new entity or dimension value appears in your data, Francis shows a notification on the relevant subsheet. To add it to your breakdown: open **Adjust breakdown**, click **Add sheet**, then drag the unassigned value onto the new sheet. You can also rename the sheet from here.
</Note>
...
<Warning>
  Removing a breakdown removes forecasts entered on the subsheets.
</Warning>
...
<AccordionGroup>
  <Accordion title="What if I want to see a combined view of a specific department across entities?">
    Use **Add roll-up** on the parent sheet and drag the relevant department subsheets from each entity into it. The roll-up shows a combined view without affecting any of the individual subsheets.
  </Accordion>
</AccordionGroup>
